use ndarray::array;
use crate::agent_v2::{DqnAgentV2, DqnAgentBuilder};
...
fn test_double_dqn_training() {
    let layers = vec![Layer::new(2, 4, Activation::Relu), Layer::new(4, 2, Activation::Relu)];
    ...
    agent.train_on_batch(&[&experience], 0.99, 0.001);
...
fn test_agent_save_load() {
    ...
    // Save agent
    let path = "test_agent_v2.bin";
    agent.save(path).unwrap();
    
    // Load agent
    let loaded_agent = DqnAgentV2::load(path).unwrap();
